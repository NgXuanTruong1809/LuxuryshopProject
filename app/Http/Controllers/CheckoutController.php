...
class CheckoutController extends Controller
{
    ...
    public function save_cart(Request $request){
        ...
        $idHoaDon = DB::table('tbl_saleorder')->max('id') + 1;
        ...
        DB::table('tbl_saleorder')->insert(
            array(
                ...
                "status" => $thongtintuuser["paymentstatus"]
            )
        );
        ...
        //Gửi mail
        $name_mail = $thongtintuuser["customerName"];
        $receive_mail = $thongtintuuser["customerEmail"];
        Mail::send('front-end.sendmail', compact('name_mail'), function($email) use($name_mail, $receive_mail){
            $email->subject('Xác nhận đơn hàng');
            $email->to($receive_mail, $name_mail);
        }
        );
